...
(x_train, y_train) = Preprocess(load('train_32x32.mat')).rgb2gray().normalize().scale().flatten().get()

(x_test, y_test) = Preprocess(load('test_32x32.mat')).rgb2gray().normalize().scale().flatten().get()
